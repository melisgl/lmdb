...
- [1 `LMDB` ASDF System][6049]
- [2 Links][0090]
- [3 Introduction][3bc3]
- [4 Design and implementation][c362]
    - [4.1 Safety][af06]
    - [4.2 Deviations from the C lmdb API][84b1]
- [5 Library versions][1d55]
- [6 Environments][d0ca]
    - [6.1 Environments reference][bd3d]
    - [6.2 Opening and closing environments][82fb]
    - [6.3 Miscellaneous environment functions][d66b]
- [7 Transactions][bae2]
    - [7.1 Nesting transactions][6ecf]
- [8 Databases][97e3]
    - [8.1 The unnamed database][a14f]
    - [8.2 `DUPSORT`][186b]
    - [8.3 Database API][67cb]
- [9 Encoding and decoding data][03b4]
    - [9.1 Overriding encodings][60cd]
- [10 Basic operations][f63b]
- [11 Cursors][eb36]
    - [11.1 Positioning cursors][471b]
    - [11.2 Basic cursor operations][ea6c]
    - [11.3 Miscellaneous cursor operations][328c]
- [12 Conditions][edfb]
    - [12.1 Conditions for C lmdb error codes][58a4]
    - [12.2 Additional conditions][5538]
...
- Version: 0.1
- Description: Bindings to [`LMDB`][6049], the Lightning Memory-mapped Database.
...
It is a small C library ("C lmdb" from now on), around which [`LMDB`][6049] is
...
[`STRING`][4267] (which are often used as keys so sorting must work as
...
- C [`LMDB`][6049] allows read transactions to be used in multiple threads.
...
- [function] **OPEN-ENV** *PATH &KEY (CLASS \*ENV-CLASS\*) (IF-DOES-NOT-EXIST :ERROR) (SYNCHRONIZED T) (MAX-DBS 1) (MAX-READERS 126) (MAP-SIZE (\* 1024 1024)) (MODE 436) (SUBDIR T) (SYNC T) (META-SYNC T) READ-ONLY (TLS T) (READ-AHEAD T) (LOCK T) (MEM-INIT T) FIXED-MAP WRITE-MAP MAP-ASYNC*
    ...
    Create an [`ENV`][412f] object through which the [`LMDB`][6049] environment can be
...
The [`LMDB`][6049] environment supports transactional reads and writes. By
...
An [`LMDB`][6049] parent transaction and its cursors must not issue operations
...
[`LMDB`][6049] has a default, unnamed database backed by a B+ tree. This db
...
A prominent feature of [`LMDB`][6049] is the ability to associate multiple
...
- [function] **GET-DB** *NAME &KEY (CLASS \*DB-CLASS\*) (ENV \*ENV\*) (IF-DOES-NOT-EXIST :CREATE) KEY-ENCODING VALUE-ENCODING INTEGER-KEY REVERSE-KEY DUPSORT INTEGER-DUP REVERSE-DUP DUPFIXED*
    ...
    may not be [`EQ`][a1d4]). The first time `GET-DB` is called with any given name
...
[`LMDB`][6049] exposes this full flexibility while at the same time providing
...
- [type] **ENCODING**
    ...
    - `:OCTETS:` Note the plural. Data to be encoded (e.g. `KEY` argument of
      ...
      required. For example, [`VECTOR`][e2a3]s can be used as long as the
    ...
    - A [`CONS`][e3aa]: Data is encoded by the function in the [`CAR`][8c99] of the
      cons and decoded by the function in the [`CDR`][2570]. For example, `:UINT64`
...
- [type] **OCTETS** *&OPTIONAL (SIZE '\*)*
    ...
    A 1D [`SIMPLE-ARRAY`][2917] of `(UNSIGNED-BYTE 8)`.
...
- [function] **G3T** *DB KEY*
    ...
    having to shadow `CL:GET` when importing the [`LMDB`][6049] package. On the
...
- [function] **CURSOR-DEL** *CURSOR &KEY DELETE-DUPS*
    ...
    current key. With `DELETE-DUPS`, [`CURSOR-DB`][3d6a] must have [`DUPSORT`][186b], else
...
- [function] **CURSOR-RENEW** *&OPTIONAL CURSOR*
    ...
    with to avoid allocation overhead. [`CURSOR-DB`][3d6a] stays the same. This
...
- [function] **CURSOR-COUNT** *&OPTIONAL CURSOR*
    ...
    `CURSOR`. If [`CURSOR-DB`][3d6a] doesn't have [`DUPSORT`][186b], [`LMDB-INCOMPATIBLE-ERROR`][12a5]
...
- [condition] **LMDB-SERIOUS-CONDITION** *SERIOUS-CONDITION*
    ...
    The base class of all [`LMDB`][6049] conditions. Conditions
...
- [condition] **LMDB-ERROR** *LMDB-SERIOUS-CONDITION ERROR*
    ...
    Base class for normal, recoverable [`LMDB`][6049] errors.
...
- [condition] **LMDB-INVALID-ERROR** *LMDB-SERIOUS-CONDITION*
    ...
    File is not a valid [`LMDB`][6049] file.
...
- [condition] **LMDB-INCOMPATIBLE-ERROR** *LMDB-ERROR*
    ...
    Operation and [`DB`][3a5d] incompatible, or `DB` type changed.
    This can mean:
...
- [condition] **LMDB-ILLEGAL-ACCESS-TO-PARENT-TXN-ERROR** *LMDB-ERROR*
    ...
    has active child transactions. In [`LMDB`][6049], [Basic operations][f63b] are
  ...
  [00c7]: #x-28LMDB-3A-40ACTIVE-TRANSACTION-20MGL-PAX-3AGLOSSARY-TERM-29 "LMDB:@ACTIVE-TRANSACTION MGL-PAX:GLOSSARY-TERM"
  ...
  [2917]: http://www.lispworks.com/documentation/HyperSpec/Body/t_smp_ar.htm "SIMPLE-ARRAY TYPE"
  ...
  [3d6a]: #x-28LMDB-3ACURSOR-DB-20FUNCTION-29 "LMDB:CURSOR-DB FUNCTION"
  ...
  [6049]: #x-28-22lmdb-22-20ASDF-2FSYSTEM-3ASYSTEM-29 '"lmdb" ASDF/SYSTEM:SYSTEM'
  ...
  [d997]: #x-28LMDB-3A-40DEFAULT-CURSOR-20MGL-PAX-3AGLOSSARY-TERM-29 "LMDB:@DEFAULT-CURSOR MGL-PAX:GLOSSARY-TERM"
  ...
  [e3aa]: http://www.lispworks.com/documentation/HyperSpec/Body/t_cons.htm "CONS TYPE"
